	...
	*/
	function get_item_search_suggestions_without_variations($search,$deleted = 0,$limit=25,$price_field = FALSE,$hide_inactive = false)
	{
		...
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
			...
			$this->db->select("items.item_id, item_number, item_images.image_id, items.name, categories.name as category, MATCH (".$this->db->dbprefix('items').".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, item_images.image_id, items.name, categories.name as category'); 
			$this->db->from('categories');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
		...
		else
		{
			...
			$this->db->select('items.item_id, items.name, item_images.image_id, categories.name as category');
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.name as item_name,items.item_id, item_images.image_id, categories.name as category');
			$this->db->from('categories');
			$this->db->like('categories.name', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->join('items', 'items.category_id=categories.id');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, items.item_number, item_images.image_id, categories.name as category');
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, items.product_id, COALESCE(phppos_items.main_image_id,phppos_item_images.image_id) as image_id, categories.name as category');
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, item_images.image_id, categories.name as category');
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			$this->db->where('items.item_id', $search);
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, item_images.image_id, items.name, additional_item_numbers.item_number');
			$this->db->from('additional_item_numbers');
			$this->db->join('items', 'items.item_id = additional_item_numbers.item_id','left');
			$this->db->like('additional_item_numbers.item_number', $search,!$this->config->item('speed_up_search_queries') ? 'both' : 'after');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
			...
			$this->db->select('items.item_id, items.name as item_name, item_images.image_id, tags.name as tag_name,items.unit_price,items.cost_price');
			$this->db->from('items_tags');
			$this->db->join('tags', 'items_tags.tag_id=tags.id');
			$this->db->join('items', 'items_tags.item_id=items.item_id');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
    ...
    $query = array(); 	
      ...
      if ($price_field) {
        $this->db->select('items.'. $price_field); 
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,item_images.image_id, item_variations.id as item_variation_id, categories.name as category, MATCH (".$this->db->dbprefix('items').".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			$this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
      ...
      if ($price_field) {
        $this->db->select('items.'. $price_field); 
			...
			$this->db->select("$quantity_field as quantity,item_variations.item_number as item_variation_number,item_variations.deleted as variation_deleted,item_images.image_id,item_variations.id as item_variation_id, categories.name as category, MATCH (".$this->db->dbprefix('items').".item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".item_number = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
      ...
      if ($price_field) {
        $this->db->select('items.'. $price_field);
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,item_images.image_id, item_variations.id as item_variation_id, categories.name as category,MATCH (product_id) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".product_id = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
      ...
      if ( $this->Item->is_array_full($suggestions, $limit) )
      {
        ...
        /* NEXT JOIN PREDICATE IS NOT SIMPLIFIED:
          $this->db->join('item_images', '('.$items_table.'.item_id = '.$item_images_table.'.item_id and '.$item_variations_table.'.id='.$item_images_table.'.item_variation_id) or ('.$items_table.'.item_id = '.$item_images_table.'.item_id and '.$item_images_table.'.item_variation_id IS NULL)', 'left');
          (`phppos_items`.`item_id` = `phppos_item_images`.`item_id` AND `phppos_item_variations`.`id` = phppos_item_images.item_variation_id)
       OR (`phppos_items`.`item_id` = `phppos_item_images`.`item_id` AND `phppos_item_images`.`item_variation_id` IS NULL)

          SIMPLIFIED: Has one AND less.
          `phppos_items`.`item_id` = `phppos_item_images`.`item_id`
          AND (`phppos_item_variations`.`id` = phppos_item_images.item_variation_id
            OR `phppos_item_images`.`item_variation_id` IS NULL)

        */
        $this->db->join('item_images', $items_table.'.item_id = '.$item_images_table.'.item_id and ('.$item_variations_table.'.id='.$item_images_table.'.item_variation_id or '.$item_images_table.'.item_variation_id IS NULL)', 'left');
...
from
	(select
		...
		phppos_item_images.image_id,
		phppos_categories.name AS category
	from `phppos_items`
    LEFT JOIN `phppos_item_variations` ON `phppos_items`.`item_id` = `phppos_item_variations`.`item_id`
	  LEFT JOIN `phppos_item_images` ON  `phppos_items`.`item_id` = `phppos_item_images`.`item_id` AND (`phppos_item_variations`.`id` = phppos_item_images.item_variation_id OR `phppos_item_images`.`item_variation_id` IS NULL)
...
SQL;
      ...
      // Query 2
			...
			$this->db->select("$quantity_field as quantity, item_variations.item_number as item_variation_number, item_variations.deleted as variation_deleted, items.item_id, items.name, items.item_number, items.product_id, item_variations.id as item_variation_id, item_images.image_id,categories.name as category", false);
      // $this->db->select("$quantity_field as quantity, item_variations.deleted as variation_deleted, item_variations.id as item_variation_id, item_images.image_id, categories.name as category", false);
      ...
      if ($price_field) {
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,items.item_id, items.name, items.item_number, items.product_id, item_variations.id as item_variation_id, item_images.image_id, categories.name as category", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
				...
				if ($search)
				{
						...
						|| $fields == $this->db->dbprefix('items').'.cost_price' || $fields == $this->db->dbprefix('items').'.unit_price' || $fields == $this->db->dbprefix('items').'.promo_price' || $fields == $this->db->dbprefix('tags').'.name' || $fields == $this->db->dbprefix('suppliers').'.company_name' || $fields == $this->db->dbprefix('manufacturers').'.name')
					{
						...
						elseif($fields == $this->db->dbprefix('tags').'.name' )
						{
							...
							$search_overall_query = "WHERE tags LIKE \"%".$this->db->escape_like_str($search).'%"';			
						}
						else
						{
